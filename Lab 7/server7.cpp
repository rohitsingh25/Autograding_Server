...
int main(int argc, char* argv[]) 
{
    ...
    while(true) 
    {
        ...
        if (system("rm student_code.cpp") != 0) {
        ...
        if (compileResult != 0) 
        {
            ...
            close(clientSocket);
            errorFile.close();
            if (system("rm compile_error.txt") != 0) {
                cerr << "Error deleting temporary code file." << endl;
            }
            continue;
        }
        ...
        if (executionResult != 0) 
        {
            ...
            close(clientSocket);
            err.close();
            if (system("rm runtime_error.txt") != 0) {
                cerr << "Error deleting temporary code file." << endl;
            }
            continue;
        }
        ...
        if(response=="OUTPUT ERROR") 
        {
            ...
            close(clientSocket);
            err.close();
            if (system("rm output_error.txt") != 0) {
                cerr << "Error deleting temporary code file." << endl;
            }
        }
        else
        {
            ...
            send(clientSocket, response.c_str(), response.size(), 0);  close(clientSocket); 
        }
        if (system("rm student_output.txt") != 0) {
            cerr << "Error deleting temporary code file." << endl;
        }
    }        
    return 0;
